...
import { useCurrentCcy, useDispatch, useLiveData } from '../../context/AppContext.jsx';
...
const NAV_ITEMS = [
  ...
  { num: 7,  label: 'POSITIONING' },
  { num: 8,  label: 'TRADE IDEAS' },
  { num: 9,  label: 'AI ANALYST' },
  { num: 10, label: 'EXECUTION' },
  { num: 11, label: 'UPDATE ASSISTANT' },
  { num: 12, label: 'NEWS FEED' },
];

function NavBadge({ text, color = 'var(--gold)' }) {
  return (
    <span style={{
...
export function Sidebar() {
  ...
  // Count unread high-impact calendar events for badge
  const calEvents   = live.calendar[cur] || [];
  const upcomingHigh = calEvents.filter(e => e.impact === 'high' && !e.actual).length;

  // News count
  const newsCount = (live.news[cur] || []).length;
    ...
    <aside className="sidebar">
      ...
      <div className="sb-score">
        ...
        {isGold ? (
          ...
          <>
            <div className="sb-rate-lbl">BENCHMARK RATE <Dot src={d.rateSrc} /></div>
        ...
        <div className="sb-div-lbl">{isGold ? 'STRENGTH SCORE' : 'DIVERGENCE SCORE'}</div>
      ...
      {!isGold && NAV_ITEMS.map(item => (
        ...
        >
          ...
          {item.num === 5 && upcomingHigh > 0 && (
            <NavBadge text={`${upcomingHigh} HIGH`} color="var(--red)" />
          )}
          {item.num === 12 && newsCount > 0 && live.status.news === 'live' && (
            <NavBadge text={`${newsCount}`} color="var(--blue)" />
          )}
        </a>
      ))}

      {/* G10 strength ranking */}
